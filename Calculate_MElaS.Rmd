...
This code will use a formula and calculate MElaS for each metabolites for each organ under different conditions. MElaS values will be added to a csv called All_MElaS_Data.csv file inclduing FC at 0w, 3w, 12w, lnFC at 3v0, 12v3 and 24v12 of all organs. 
...
for (path in nested_dir) {
  ...
  #Add LogFC of 3v0, 12v3 and 24v12 to MElaS file per organ
  for(tissi in TissTitle){
...
write.csv(combined_df, "All_MElaS_Data.csv", row.names = FALSE)
```
